<template>
  <div class="app-container">
    ...
    />
    <position-add :form-visible="addFormVisible" :passed-data="levelList" @addSuccess="addSuccess" @addVisibleChange="addVisibleChange" />
    <position-update :form-visible="updateFormVisible" :passed-data="levelList" :rowdata="rowData" @updateSuccess="updateSuccess" @updateVisibleChange="updateVisibleChange" />
...
<script>
import { getPositionList, positionDelete } from '@/api/position'
  ...
  methods: {
    ...
    handleDelete(index, id) {
      this.$confirm('将删除该条信息, 是否确定?', '提示', {
        ...
        .then(() => {
          console.log('id:', [id])
          positionDelete({ id: [id] }).then(response => {
            this.$message({
    ...
    deleteMutiData() {
      this.$confirm('将删除选中信息, 是否确定?', '提示', {
        ...
        positionDelete({ id: this.multipleSelection.map(item => item.id) }).then(response => {
          this.$message({
            message: response.message,
            type: 'success'
          })
