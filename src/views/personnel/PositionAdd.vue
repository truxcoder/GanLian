<template>
  <el-dialog v-loading="dialogLoading" title="添加职务信息" :width="dialogWidth" :visible.sync="formVisible" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
...
<script>
import { positionAdd } from '@/api/position'
    ...
    onSubmit() {
      this.$refs.addForm.validate(valid => {
        if (valid) {
          ...
          positionAdd(this.form).then(response => {
            this.$message({
