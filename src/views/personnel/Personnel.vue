  ...
  <div class="app-container">
    ...
    <div class="tool-bar">
      ...
      <el-button
        v-if="can.manage"
        type="primary"
        icon="el-icon-scjd-circle-forbidden iconfont"
        size="mini"
        @click="fetchDisabledData"
      >
        查看禁用人员
      </el-button>
      <el-button
        v-if="can.special"
        v-show="!quickSearchBoxShow"
        type="success"
        icon="el-icon-arrow-down"
        size="mini"
        @click="onQuickSearchBoxShow"
      >
        展开快捷查询
      </el-button>
      <el-button
        ...
        <div class="border pb-4 mb-4">
          ...
          <div class="quick-buttons">
            <el-button
              ...
              type="success"
              plain
              @click="onQuickSearch('willUpInSixMonth')"
            >
              半年内将晋升职级的人员
            </el-button>
            <el-popover
          ...
          <div class="quick-buttons">
            <el-button
              ...
              type="success"
              plain
              @click="onQuickSearch('willUpInThreeMonth')"
            >
              三个月内将晋升职级的人员
            </el-button>
            <el-popover
          ...
          <div class="quick-buttons">
            <el-button
              ...
              type="success"
              plain
              @click="onQuickSearch('willRetireInTwoYear')"
            >
              两年内将退休的人员
            </el-button>
            <el-popover
          ...
          <div class="quick-buttons">
            <el-dropdown
              class="data-export"
              @command="handleQuickSearchCommand"
            >
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="willUp2d">
                  二级调研员
                </el-dropdown-item>
                <el-dropdown-item command="willUp3d">
                  三级调研员
                </el-dropdown-item>
                <el-dropdown-item command="willUp4d">
                  四级调研员
                </el-dropdown-item>
                <el-dropdown-item command="willUp1z">
                  一级主任科员
                </el-dropdown-item>
                <el-dropdown-item command="willUp2z">
                  二级主任科员
                </el-dropdown-item>
                <el-dropdown-item command="willUp3z">
                  三级主任科员
                </el-dropdown-item>
                <el-dropdown-item command="willUp4z">
                  四级主任科员
                </el-dropdown-item>
                <el-dropdown-item command="willUp1k">
                  一级科员
                </el-dropdown-item>
                <el-dropdown-item command="willUp2g">
                  二级高级警长
                </el-dropdown-item>
                <el-dropdown-item command="willUp3g">
                  三级高级警长
                </el-dropdown-item>
                <el-dropdown-item command="willUp4g">
                  四级高级警长
                </el-dropdown-item>
                <el-dropdown-item command="willUp1j">
                  一级警长
                </el-dropdown-item>
                <el-dropdown-item command="willUp2j">
                  二级警长
                </el-dropdown-item>
                <el-dropdown-item command="willUp3j">
                  三级警长
                </el-dropdown-item>
                <el-dropdown-item command="willUp4j">
                  四级警长
                </el-dropdown-item>
                <el-dropdown-item command="willUp1y">
                  一级警员
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
    ...
    >
      ...
      <el-table-column label="所属单位" align="center" width="120">
        <template slot-scope="scope">
          <span :class="{ text_red: !scope.row.organShortName }">{{
            scope.row.organShortName ? scope.row.organShortName : '未定义'
          }}</span>
      ...
      <el-table-column label="所属部门" align="center">
        <template slot-scope="scope">
          <span :class="{ text_red: !scope.row.departmentShortName }">{{
            scope.row.departmentShortName
              ? scope.row.departmentShortName
              : '未定义'
          }}</span>
          <!-- <span :class="{'text_red':!scope.row.organId}">{{ scope.row.organId ? getOrganName(scope.row.organId, "short") : '未定义' }}</span> -->
      ...
      <el-table-column align="center" label="姓名">
        <template slot-scope="scope">
          <el-link @click="handleTable(scope.$index, scope.row)">
            {{ scope.row.name }}
          </el-link>
        </template>
      </el-table-column>
      ...
      <el-table-column label="年龄" width="60" align="center">
        <template slot-scope="scope">
          <span :class="{ text_red: isBirthdayZero(scope.row.birthday) }">{{
      ...
      <el-table-column align="center" label="操作" :width="lastColumnWidth">
        <template slot-scope="scope">
          <el-button
            v-if="canRender(scope.row.organId, 'update')"
            size="mini"
            type="success"
            @click="handleEdit('update', scope.row)"
          >
            编辑
          </el-button>
          ...
          <el-button
            v-if="canRender(scope.row.organId, 'manage')"
            size="mini"
...
export default {
  ...
  created() {
    ...
    if (this.$store.getters.custom.length === 0) {
      this.$store.dispatch('account/changeCustom', {
        category: 1,
  ...
  methods: {
    ...
    handleDisable(row) {
      ...
      this.$confirm(message, '提示', {
        ...
        .then(() => {
          request('personnel', 'update_status', {
            id: row.id,
            status: !row.status,
          })
            .then((response) => {
