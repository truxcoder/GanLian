...
<template>
  <div class="app-container">
    <el-form ref="searchForm" :inline="true" :model="searchForm" class="demo-form-inline">
      ...
      <el-form-item label="数据分类" prop="category">
        <el-select v-model="searchForm.category" size="small" :style="searchItemWidth" placeholder="请选择分类">
          <el-option v-for="i in feedbackCategoryDict" :key="i.value" :label="i.label" :value="i.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="当前状态" prop="status">
        <el-select v-model="searchForm.status" size="small" :style="searchItemWidth" placeholder="请选择状态">
          <el-option v-for="i in feedbackStatusDict" :key="i.value" :label="i.label" :value="i.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="listLoading" :data="currentData" element-loading-text="Loading" stripe border :fit="true" highlight-current-row @selection-change="handleSelectionChange">
      ...
      <el-table-column label="审核人" align="center" prop="feedbackerName" />
    ...
    />
    <FeedbackDetail :visible="detailVisible" :row="rowData" @editSuccess="editSuccess" @visibleChange="visibleChange" />
  </div>
</template>
...
import { feedbackCategoryDict, feedbackStatusDict } from '@/utils/dict'

export default {
  name: 'Feedback',
  components: { FeedbackDetail, PersonnelOption },
  ...
  methods: {
    fetchData(data = {}, params = {}) {
      ...
      request(this.resource, 'feedback', data, params).then(response => {
        if (response.count) {
    ...
    getCategory(c) {
      for (const i of feedbackCategoryDict) {
        if (i.value === c) {
          return i.label
        }
      }
