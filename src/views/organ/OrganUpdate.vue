<template>
  <el-dialog v-loading="dialogLoading" title="编辑机构信息" :width="dialogWidth" :visible.sync="formVisible" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form v-if="formVisible" ref="updateForm" :inline="true" class="add-form" :model="form" :rules="rules" size="medium" :label-width="formLabelWidth" label-position="right">
      <el-form-item v-for="item in formItemData" :key="item" :label="models[item].label" :prop="item">
        ...
        <el-select v-if="item=='parent'" v-model="form[item]" :style="formItemWidth" :placeholder="'请选择'+models[item].label">
          <el-option v-for="i in wholeOrganList" :key="i.id" :label="i.name" :value="i.id" />
        </el-select>
...
<script>
import { organUpdate } from '@/api/organ'
import { mixin } from '@/common/mixin/organ'
export default {
  name: 'OrganUpdate',
  mixins: [mixin],
  props: {
    ...
    onSubmit() {
      this.$refs.updateForm.validate(valid => {
        if (valid) {
          ...
          organUpdate(this.form).then(response => {
            this.$message({
