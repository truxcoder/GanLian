 ...
 * @LastEditTime: 2024-06-11 08:45:19
 * @LastEditors: truxcoder
 * @Description: 高级警长配置管理
-->
<template>
  <div class="app-container">
    <div class="tableZone">
      <el-table v-loading="listLoading" :data="queryMeans === 'backend' ? currentData : currentPageData" element-loading-text="Loading" stripe :height="height" :fit="true" highlight-current-row>
        <el-table-column align="center" label="名称" prop="shortName" />
        <el-table-column align="center" label="四高(占用数/编制数)">
          <template slot-scope="scope">
            <div v-if="currentEditIndex !== scope.$index" class="flex justify-center items-center content-center">
              <div :class="getStyle(scope.row, 'g4')">{{ getUse(scope.row.id, 'g4') }}</div>
              <div class="right-button">{{ getNumber(scope.row.position, 'g4') }}</div>
            </div>
            <!-- <span v-if="currentEditIndex !== scope.$index">{{ getUse(scope.row.id, 'zc') }} / {{ getNumber(scope.row.position, 'zc') }}</span> -->
            <el-input v-else v-model.number="form.g4" size="mini" class="w-14" />
          </template>
        </el-table-column>
        <el-table-column align="center" label="三高(占用数/编制数)">
          <template slot-scope="scope">
            <div v-if="currentEditIndex !== scope.$index" class="flex justify-center items-center content-center">
              <div :class="getStyle(scope.row, 'g3')">{{ getUse(scope.row.id, 'g3') }}</div>
              <div class="right-button">{{ getNumber(scope.row.position, 'g3') }}</div>
            </div>
            <!-- <span v-if="currentEditIndex !== scope.$index">{{ getUse(scope.row.id, 'zc') }} / {{ getNumber(scope.row.position, 'zc') }}</span> -->
            <el-input v-else v-model.number="form.g3" size="mini" class="w-14" />
          </template>
        </el-table-column>
        <el-table-column align="center" label="二高(占用数/编制数)">
          <template slot-scope="scope">
            <div v-if="currentEditIndex !== scope.$index" class="flex justify-center items-center content-center">
              <div :class="getStyle(scope.row, 'g2')">{{ getUse(scope.row.id, 'g2') }}</div>
              <div class="right-button">{{ getNumber(scope.row.position, 'g2') }}</div>
            </div>
            <el-input v-else v-model.number="form.g2" size="mini" class="w-14" />
          </template>
        </el-table-column>
        <el-table-column align="center" label="一高(占用数/编制数)">
          <template slot-scope="scope">
            <div v-if="currentEditIndex !== scope.$index" class="flex justify-center items-center content-center">
              <div :class="getStyle(scope.row, 'g1')">{{ getUse(scope.row.id, 'g1') }}</div>
              <div class="right-button">{{ getNumber(scope.row.position, 'g1') }}</div>
            </div>
            <el-input v-else v-model.number="form.g1" size="mini" class="w-14" />
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    fetchData(data = {}, params = {}) {
      ...
      request('department', 'high_level', data, params).then((response) => {
        if (response.data) {
    ...
    onUpdateSubmit(index, row) {
      ...
      request('department', 'update', data, params)
        .then((response) => {
